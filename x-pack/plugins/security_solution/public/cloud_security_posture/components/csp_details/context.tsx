...
export interface CspDetailsContext {
  /**
   * Id of the document
   */
  eventId: string;
 ...
 */
export const CspDetailsContext = createContext<CspDetailsContext | undefined>(undefined);

export type CspDetailsProviderProps = {
  /**
...
export const CspDetailsProvider = memo(
  ({ id, indexName, scopeId, isPreviewMode, children, field, query }: CspDetailsProviderProps) => {
    ...
    return <CspDetailsContext.Provider value={contextValue}>{children}</CspDetailsContext.Provider>;
  }
);

CspDetailsProvider.displayName = 'CspDetailsProvider';

export const useCspDetailsContext = (): CspDetailsContext => {
  const contextValue = useContext(CspDetailsContext);

  if (!contextValue) {
    throw new Error('CspContext can only be used within CspContext provider');
  }
